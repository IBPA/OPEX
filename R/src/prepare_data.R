#' This module includes the wrapper function "prepare_data" and four internal
#' functions called by it. The input of the wrapper function is a Simulator 
#' object. The four functions in the wrapper, set up the path for storing the
#' results after running a simulation.

#' Create the directory for storing the results of a simulation based on the 
#' setting of the Simulator object.
...
set_res_path <- function(simulator) {
  ...
  simulator$res_path <- file.path(out_data_path, folder,
                                  simulator$setting["method"])
  cat("create folder")
...
#' Create the name of the folder for storing the results based on the setting 
#' of the simulator object.

set_res_name <- function(simulator){
  simulator$res_name <- paste("every", simulator$setting["batch_size"],
                              "iter_num", simulator$setting["iter_num"],
                              sep = "_")
}


#' Load and sort the data, the first 14 rows covers all the antispetics and 
#' biocides.
...
load_data <- function(simulator) {
  ...
  start_condition <- c(paste(antiseptics[row_index], antibiotics[col_index], 
                             sep = "_"), 
                      "Control")
...
#' Split the 45 culture conditions into two parts, the starting training set 
#' and a pool from which the next dataset is sampled. The pool is also used 
#' for evaluating the prediction performance of the GP models.
...
#' Prepare data for the given simulator
